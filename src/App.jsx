      ...
      {open && (
        <div style={{ padding:"0 16px 20px", borderTop:`1px solid ${C.border}` }}>
          ...
          <div style={{ marginTop:16 }}>
            <SHead title={`Google Shopping (${raw.shopping?.results?.length || 0} results)`} col={C.navy} />
            {raw.shopping?.results?.length > 0 ? (
              <div style={{ overflowX:"auto" }}>
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                  <thead>
                    <tr style={{ background:C.light }}>
                      {["#","Product","Price","Seller","Rating","Delivery"].map(h => (
                        <th key={h} style={{ padding:"6px 8px", textAlign:"left", color:C.mid, fontWeight:600, borderBottom:`1px solid ${C.border}`, whiteSpace:"nowrap" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {raw.shopping.results.map((r, i) => (
                      <tr key={i} style={{ borderBottom:`1px solid ${C.border}` }}>
                        <td style={{ padding:"6px 8px", color:C.mid, fontWeight:700 }}>{r.position}</td>
                        <td style={{ padding:"6px 8px", color:C.dark, maxWidth:220, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }} title={r.title}>{r.title}</td>
                        <td style={{ padding:"6px 8px", color:C.teal, fontWeight:700 }}>{r.price||"—"}</td>
                        <td style={{ padding:"6px 8px", color:C.gray }}>{r.seller||"—"}</td>
                        <td style={{ padding:"6px 8px", color:C.gray, whiteSpace:"nowrap" }}>{r.rating ? `${r.rating}★ (${r.reviews||0})` : "—"}</td>
                        <td style={{ padding:"6px 8px", color:C.mid, fontSize:11 }}>{r.delivery||"—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div style={{ fontSize:13, color:C.mid, fontStyle:"italic" }}>
                No shopping results returned.{raw.shopping?.error && ` (${raw.shopping.error})`}
              </div>
            )}
          </div>
          ...
          <div style={{ marginTop:16 }}>
            ...
            <div style={{ fontSize:13, color:C.gray, marginBottom:8 }}>
              Active ads: <strong>{raw.ads?.count || 0}</strong>
              {raw.ads?.verified && <span style={{ marginLeft:10, color:C.teal, fontWeight:600 }}>✓ Verified advertiser</span>}
              {raw.ads?.regions?.length > 0 && <span style={{ marginLeft:10, color:C.mid }}>Regions: {raw.ads.regions.join(", ")}</span>}
            </div>
            {raw.ads?.ads?.length > 0 ? (
              raw.ads.ads.map((a, i) => (
              ...
              <div style={{ fontSize:13, color:C.mid, fontStyle:"italic" }}>
                No ad creatives found.{raw.ads?.error && ` (${raw.ads.error})`}
              </div>
            )}
          </div>
          ...
          <div style={{ marginTop:16 }}>
            ...
            {raw.serp?.shopping_ads?.length > 0 && (
              <div style={{ marginBottom:8 }}>
                <div style={{ fontSize:12, color:C.mid, marginBottom:4 }}>Shopping ads at top:</div>
...
export default function App() {
  ...
  const LOAD_MSGS = [
    "Querying Google Shopping for product category...",
    "Pulling competitor pricing data...",
    "Checking Google Ads Transparency Center...",
    "Running SERP analysis...",
    "Interpreting data and building brief...",
  ];
