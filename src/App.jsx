      ...
      {open && (
        <div style={{ padding:"0 16px 20px", borderTop:`1px solid ${C.border}` }}>
          ...
          <div style={{ marginTop:14, background:C.navyL, borderRadius:4, padding:"10px 14px", fontSize:12 }}>
            <div style={{ fontWeight:700, color:C.navy, marginBottom:6 }}>Queries generated by Claude (after visiting website)</div>
            {raw.product_summary && (
              <div style={{ color:C.navy, fontStyle:"italic", marginBottom:8, paddingBottom:8, borderBottom:`1px solid ${C.border}` }}>
                Website read: {raw.product_summary}
              </div>
            )}
            <div style={{ color:C.gray }}>Q1 Brand: <strong>"{queries.q1}"</strong> — finds prospect's own listings</div>
            <div style={{ color:C.gray }}>Q2 Category: <strong>"{queries.q2}"</strong> — specific category competitors</div>
...
export default function App() {
  const [form, setForm] = useState({
    name:"", email:"", phone:"", website:"", goal:"", details:"",
  });
    ...
    <div style={{ minHeight:"100vh", background:"#F8F8F9", fontFamily:"'DM Sans','Helvetica Neue',Helvetica,sans-serif" }}>
      ...
      <div style={{ maxWidth:920, margin:"0 auto", padding:"28px 20px" }}>
        ...
        <div style={{ background:C.navyL, borderLeft:`4px solid ${C.navy}`, borderRadius:4, padding:"12px 16px", marginBottom:22, fontSize:13, color:C.navy }}>
          <strong>How this works:</strong> Paste the prospect's intake form. Claude visits their website to understand exactly what they sell, then runs five live searches — three targeted Google Shopping searches (brand presence, category competitors, specific product pricing), Google Ads Transparency for their active ad creatives, and SERP for branded presence — then interprets all the real data to produce a data-backed pre-call brief. Output maps to Document 2 of the Sales System.
        </div>
